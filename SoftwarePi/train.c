...
int train_cmd(char* arg) 
{
	...
	int sector;
	sector = atoi (arg + strlen("wait "));
	while (train_get_current_sector(current_train) != sector) { 
	    usleep (20000);
	}
	return 0;
    }

    if (0 == strncmp(arg, "est_time ", strlen("est_time "))) {
	int sector;
	sector = atoi(arg + strlen("est_time "));
        //XXX
	return 0;
    }
	...
	int max_error;
	max_error = atoi(arg + strlen("check_est "));
        //XXX
	return 0;
    }
        
    printf ("Incorrect command. "
	    "Use train help to see a list of available commands\n");
    return 1;
}
...
char train_get_current_sector (train_t* this)
{
