...
float get_initial_coarse_estimation(int current_speed, int target_speed, float sector_length, int cv3, int vmin, int vmid, int vmax ){
	...
	float acceleration_time=acceleration_time_step*(target_speed-current_speed);
	...
	for(i=current_speed;i<target_speed;i++){
		distance_accelerating+=(speed_table[i+1]+speed_table[i])*acceleration_time_step/2;
	}
	...
	if(speed_table[target_speed]<=0) return -1;
